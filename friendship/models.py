...
class FriendshipRequest(models.Model):
    ...
    def __str__(self):
        return f"User #{self.from_user} friendship requested #{self.to_user}"
...
class FriendshipManager(models.Manager):
    ...
    def add_friend(self, from_user, to_user, message=None):
        ...
        if Block.objects.filter(
            blocker=from_user, blocked=to_user
        ).exists():
            raise AlreadyExistsError(_("You blocked this user."))
        
        if Block.objects.filter(
            blocker=to_user, blocked=from_user
        ).exists():
            raise AlreadyExistsError(_("User blocked you."))
...
class FollowingManager(models.Manager):
    ...
    def add_follower(self, follower, followee):
        ...
        if created is False:
            raise AlreadyExistsError(_(f"User '{follower}' already follows '{followee}'"))
...
class BlockManager(models.Manager):
    ...
    def add_block(self, blocker, blocked):
        ...
        if created is False:
            raise AlreadyExistsError(_(f"User '{blocker}' already blocks '{blocked}'"))
...
class Block(models.Model):
    ...
    def __str__(self):
        return f"User #{self.blocker} blocks #{self.blocked}"
